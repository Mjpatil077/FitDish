...
const CategoryPage = () => {
  ...
  const fetchCategoryData = async () => {
    ...
    try {
      ...
      // 2️⃣ Fetch videos for THIS category (IMPORTANT FIX)
      const videosRes = await axios.get(`/api/videos?category=${slug}`);
      setVideos(videosRes.data.videos || []);

      // 3️⃣ (Optional) Chefs – disabled for now
      setChefs([]);
    ...
    <div className="bg-secondary-50 min-h-screen">
      ...
      <section className="bg-gradient-to-br from-primary-50 to-secondary-100">
        <div className="container-custom">
          ...
          <div className="pb-12 text-center md:text-left">
            <span className="px-4 py-2 bg-primary-100 text-primary-700 rounded-full text-sm font-medium inline-block mb-4">
              {category.type || 'Cuisine'}
            </span>
